...
- **ffmpeg/ffprobe** via Homebrew: `brew install ffmpeg` (used for ingestion, Air Spec, and the signal monitor).
- Internet access to the public HLS feeds and to `smtp.gmail.com:465`, if the signal monitor is on.
...
| `monitor.enabled` | Signal monitor on/off (also the switch on the **Signal Monitor** tab). Watches each feed in `monitor.sources` (default: the seven GLC public HLS feeds) and e-mails `email.recipients` when one stays black for `monitor.black.alertAfterSeconds` (default 10s), silent below `monitor.silence.thresholdDb` (-50 dB) for `monitor.silence.alertAfterSeconds` (30s), or sends nothing for `monitor.down.alertAfterSeconds` (default 60s). A feed linked to an OTAV channel first gets a scheduler resync (`monitor.resync`, on by default, at most once per 15 min per channel) and only alerts if the picture is still black 60s later. Every setting is editable from the tab. |
| `email.{user,appPassword,fromName,recipients}` | Gmail account the alerts are sent from. `appPassword` is a Google **App Password** (needs 2-Step Verification on the account), not the account password. `host`/`port` override `smtp.gmail.com:465` for another provider. |
...
> **Security note:** SMB, OTAV and Gmail app-password credentials are stored in plaintext in
