...
- **Movie blocks:** a `BlockTemplate` with `is_movie_block = 1` bypasses the per-series cycle entirely. It pools every title its assigned series expose, drops anything still cooling down or already scheduled within ±6 days of the target date (no repeats inside a week), then searches that pool for the ordered run of up to `movie_limit` titles (NULL = `config.movies.maxPerBlock`, default 2) whose quarter-hour-aligned span fills the slot best. This is what keeps a long slot from becoming one feature plus hours of filler. `max_per_show` does not apply to a movie block — `movie_limit` is its cap. Two titles of the same franchise may share a block only in ascending part order; standalone films (chapter 0) carry no order and pair freely.
