...
- **Operator override, for a block with no solution.** Some blocks cannot be fixed by editing: a three-hour documentary slot whose every remaining episode runs 50 minutes leaves a 30-minute hole no clip in the catalogue fills. `POST /api/blocks/:id/override` sets `ScheduledBlock.override_reason` to the validation line as it reads at that moment (plus the operator's optional note) and `override_at`; the **⚠ Force this block** button in the modal is the way in. `fits` keeps meaning "passes the rules" and stays false — a forced block is amber in the modal and `forced · <problem>` on the week card, never green — while `approvable = fits || overridden` is what the approve and push gates ask. Forcing a block that already passes is refused (409): there would be nothing to force, and a stale override would hide a real problem later. **The force carries to the other airings of that day** (`siblingAirings()` — same template, channel and date, different slot): a repeat strict-mirrors the primary clip for clip, so it holds the identical problem, and forcing one while the midnight repeat stayed refused would just mean doing it again at midnight. Each sibling records its OWN validation line with the same note, since a slot can be a different length; a sibling that passes on its own is left alone. Undoing clears them together, and so does regenerating — an override is a judgement about content, and a rebuild leaves it describing nothing.
