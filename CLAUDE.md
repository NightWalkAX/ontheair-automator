...
- **Only the FIRST main item of a block is quarter-hour aligned** (`alignGap()`); everything after it runs straight on. Aligning every item quantised the schedule to 15 minutes and was the biggest single source of filler on the real catalogue: an 8-minute episode in a 30-minute slot cost 7 minutes of filler and then pushed its successor past the block end, so the slot aired one programme and 21 minutes of filler where two fit with four to spare. What the schedule promises is when a BLOCK starts. `pickMovieRun()` and `chooseMovies()` score runs through the same helper, or a movie run would be chosen for a layout that no longer happens.
