 ...
 */
export function templateSeries(template) {
  const rows = db.prepare(`
    SELECT bts.subject,
  ...
  `).all(template.channel_id, template.id);
...
export function pickMainContent(template, block, blockSecs) {
  const series = templateSeries(template);
  if (!series.length) return [];

  const iters = series.map((s) => iteratorForSeries(s, template.channel_id, block, blockSecs));
  const items = [];
...
function loadBlock(blockId) {
  ...
  return { block, template, start, end, slotOrder: block.slot_order ?? 0 };
}
...
/** Re-sync all secondary airings of a template/date to match the primary block. */
function resyncMirrors(template_id, target_date, primaryBlockId) {
  const mirrors = db.prepare(
    'SELECT id FROM ScheduledBlock WHERE template_id = ? AND target_date = ? AND id != ?'
  ).all(template_id, target_date, primaryBlockId);
...
export function populateBlock(block) {
  ...
  if (!ctx) return null;
  const { template, start, end, slotOrder } = ctx;
  const blockSecs = blockDurationSeconds(start, end);

  // Secondary airing: copy the primary's content verbatim.
  if (slotOrder > 0) {
    ...
    const primary = primarySlot && db.prepare(
      'SELECT id FROM ScheduledBlock WHERE template_id = ? AND slot_id = ? AND target_date = ?'
    ).get(template.id, primarySlot.id, block.target_date);
  ...
  // Keep secondary airings identical to what we just built.
  resyncMirrors(template.id, block.target_date, block.id);
...
export function rollForwardTemplates(weekStart = new Date()) {
  ...
  const insert = db.prepare(`
    INSERT OR IGNORE INTO ScheduledBlock (template_id, slot_id, target_date, status)
    VALUES (?, ?, ?, 'draft')
  `);
  const fetch = db.prepare(
    'SELECT * FROM ScheduledBlock WHERE template_id = ? AND slot_id = ? AND target_date = ?'
  );
  ...
  for (let i = 0; i < 7; i++) {
    ...
    for (const t of templates) {
      ...
      for (const slot of templateSlots(t)) {
        insert.run(t.id, slot.id, target);
        const block = fetch.get(t.id, slot.id, target);
